...
function BookingViewPage() {
  ...
  const queryParams = useMemo<{ include: { flight: boolean; user: boolean } }>(
    () =>
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem
                  label="Booking Date"
                  text={data?.booking_date ? format(data?.booking_date, 'dd-MM-yyyy') : ''}
                />

                <FormListItem label="Seat Number" text={data?.seat_number} />
                ...
                {hasAccess('flight', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Flight"
                    text={
                      <Link as={NextLink} href={`/flights/view/${data?.flight?.id}`}>
