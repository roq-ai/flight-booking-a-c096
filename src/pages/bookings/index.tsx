  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['flight', 'user', 'ticket.count'],
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.booking_date ? format(record?.booking_date, 'dd-MM-yyyy') : '',
    },
    { id: 'seat_number', header: 'seat_number', accessorKey: 'seat_number' },
    { id: 'booking_status', header: 'Booking Status', accessorKey: 'booking_status' },
    hasAccess('flight', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'flight',
          header: 'Flight',
          accessorKey: 'flight',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/flights/view/${record.flight?.id}`}>
              {record.flight?.flight_number}
            </Link>
          ),
        }
      : null,
