...
import { useFlightCount, useAirportCount, useBookingCount } from 'lib/roq';

import { TicketListPage as ShowCaseList } from 'pages/tickets';

function DashboardPage() {
  const { session } = useSession();

  const flightStat = useFlightCount();

  const airportStat = useAirportCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Flights',
      result: flightStat?.data,
      isLoading: flightStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Airports',
      result: airportStat?.data,
      isLoading: airportStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bookings',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`tickets`} passHref legacyBehavior>
                  <Button
