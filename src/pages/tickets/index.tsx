...
import { useRoqClient, useTicketFindManyWithCount } from 'lib/roq';
...
export function TicketListPage(props: TicketListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['ticket_number.contains', 'ticket_status.contains'],
          ...params.filters,
        },
        'ticket',
      ),
  ...
  const { data, error, isLoading, mutate } = useTicketFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.ticket.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    { id: 'ticket_number', header: 'Ticket Number', accessorKey: 'ticket_number' },
    { id: 'ticket_price', header: 'ticket_price', accessorKey: 'ticket_price' },
    { id: 'ticket_status', header: 'Ticket Status', accessorKey: 'ticket_status' },
